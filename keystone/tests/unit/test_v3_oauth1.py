...
class AccessTokenCRUDTests(OAuthFlowTests):
    ...
    def test_list_access_token_roles_with_trust_token_blocked(self):
        """Trust-scoped token must not list an access token's roles.

        Divergence from upstream Gerrit 990500: upstream blocks list/get/delete
        of access tokens for delegated tokens but does not touch the
        access-token *roles* endpoints. This backport extends
        _block_delegated_token to those endpoints for consistency. Not covered
        by the upstream test suite.
        """
        self.test_oauth_flow()
        trust_token = self._make_trust_token()
        access_token_key = self.access_token.key.decode()
        url = (
            f'/users/{self.user_id}/OS-OAUTH1/access_tokens/'
            f'{access_token_key}/roles'
        )
        self.get(url, token=trust_token, expected_status=http.client.FORBIDDEN)

    def test_get_access_token_role_with_app_cred_blocked(self):
        """Application credential token must not read an access token role.

        Companion to the trust-token case above, covering the single-role
        endpoint with an application-credential token.
        """
        self.test_oauth_flow()
        app_cred = {
            'id': uuid.uuid4().hex,
        ...
        app_cred_token = self.v3_create_token(auth_data).headers[
            'X-Subject-Token'
        ]
        access_token_key = self.access_token.key.decode()
        url = (
            f'/users/{self.user_id}/OS-OAUTH1/access_tokens/'
            f'{access_token_key}/roles/{self.role_id}'
        )
        self.get(
            url,
