...
class TestCredentialTrustScoped(CredentialBaseTestCase):
    ...
    def test_trust_token_cannot_move_credential_cross_project(self):
        """Trust-scoped token must not move an in-scope credential out of scope.

        Divergence from upstream Gerrit 990500: upstream only validates the
        *existing* credential's project on PATCH. A trust token scoped to
        project A that legitimately owns a credential in project A could set
        project_id=B in the update body to move it out of the delegation's
        scope. The merged-result check added in this backport (the second
        _check_credential_project_scope call on dict(current, **credential))
        blocks that move. This case is not covered by the upstream test suite.
        """
        ...
        # A credential the trust token legitimately owns (same project).
        cred_ref = {
            'user_id': self.user_id,
            'type': 'cert',
            'project_id': self.project_id,
        ...
        r = self.post('/credentials', body={'credential': cred_ref})
        cred_id = r.result['credential']['id']

        trust_token = self._get_trust_token()
        # The existing-credential check passes (same project); the move must
        # still be rejected by the merged-result check.
        self.patch(
...
class TestCredentialEc2(CredentialBaseTestCase):
    ...
    def test_ec2_auth_access_token_null_project_blocked(self):
        """OAuth1-backed EC2 credential with a null project must not auth.

        Divergence from upstream Gerrit 990500: upstream compares
        cred.project_id != access_token.project_id with a bare ``!=``. This
        backport additionally rejects the request when either side's
        project_id is falsy (the ``not cred_project or not token_project``
        guard in EC2_S3_Resource). This test guards the null-credential-project
        path. Note: the bare ``!=`` would also reject this particular case;
        the uniquely-divergent both-null case requires a null-project OAuth1
        token, which is impractical to construct via the public API, so it is
        ...
        blob['access_token_id'] = access_key
        # Null out the credential's project anchor.
        blob['project_id'] = None
        ref['blob'] = json.dumps(blob)
        PROVIDERS.credential_api.create_credential(ref['id'], ref)
